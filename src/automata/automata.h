...
// TODO this file really needs some documentation
...
public:
	...
	using NodeRef = Node *;
	using ConstNodeRef = Node const *;
private:
	KeyType const name;
public:
	typename map_traits<C,TR>::map_type transitions;
	std::function<NodeRef(C)> defaultTransition;
	bool final;
	...
	Node(KeyType const & name): 
		...
		final(false)
	{
		defaultTransition = [](C s) -> NodeRef { return nullptr; };
	}

	KeyType const & getName() const { return name; }

	ConstNodeRef transit(C s) const
	{
	...
	{
		auto & result = transitions[s]; // TODO use map traits
		result.node = &node;
		return result;
	}
};
...
public:
	...
	FiniteAutomata(FiniteAutomata const &) = default;
	FiniteAutomata(FiniteAutomata &&) = default;
	...
	class output_iterator: public std::iterator<std::output_iterator_tag,C> {
		struct RefProxy {
			Consumer consumer;
			RefProxy(ConstNodeRef node): consumer(node) {}
			RefProxy & operator=(C s) { consumer.consume(s); return *this; }
		};

		RefProxy refProxy;
	public:
		output_iterator(ConstNodeRef node): refProxy(node) {}
		output_iterator & operator++() { return *this; }
		RefProxy & operator*() { return refProxy; }
		explicit operator bool() const { return ! refProxy.consumer.fail(); }
	};
...
class RangeSetter {
	...
	void do_setTrans(typename Automata::KeyType const & src, std::string const & ranges, typename Automata::KeyType const & dest, typename TTraits<C,TR>::output_type const * output)
	{
		...
		if( ch1 != 0 ) {	// add remainder stacked ch1
